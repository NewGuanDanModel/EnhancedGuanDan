...
# parser.add_argument('--exp_path', type=str, default='Danzero_plus-main/actor_n/log',
#                     help='Directory to save logging data, model parameters and config file')
parser.add_argument('--exp_path', type=str, default='Danzero_plus-main/actor_n/log',
                    help='Directory to save logging data, model parameters and config file')
parser.add_argument('--num_saved_ckpt', type=int, default=5,
...
class Player():
    def __init__(self, args) -> None:
        ...
        self.play_time = 0
        self.testing = False
        self.index = args.client_index

        # 模型初始化
        self.model_id = -1
        self.model = GDModel(self.args.observation_space, (5, 216))
        # self.model.load("better_model1.ckpt")
        with open('Danzero_plus-main/actor_n/train_model2_2.ckpt', 'rb') as f:
            new_weights = pickle.load(f)
            self.origin_weights = new_weights
        ...
        # self.weight_socket = context.socket(zmq.REQ)
        # self.weight_socket.connect(f'tcp://localhost:{self.args.param_port}')
        ...
        #     new_weights, self.model_id = find_new_weights(-1, "q_network.ckpt")
    ...
    def sample(self, state) -> int:
        ...
        if self.args.epsilon > 0 and np.random.rand() < self.args.epsilon:
            ...
            for i in range(size):
                if output[i] > 0.0:
                    output[i] = output[i] / state['penalty'][i]
                else:
                    output[i] = output[i] * state['penalty'][i]
                output[i] += state['addition'][i]
...
def run_one_player(index, args):
    ...
    while True:
        ...
        if not isinstance(state, int) and not isinstance(state, float) and not isinstance(state, str):
            if len(state.keys()) == 1 and 'fuck' in state.keys():
                player.testing = state['fuck']
                socket.send(serialize({'fuck' : player.testing}).to_buffer())
                if player.testing == True and player.index % 2 == 1:
                    player.model.set_weights(player.origin_weights)
                if player.testing == False and player.index % 2 == 1:
                    player.model.set_weights(player.new_weights)
            else:
